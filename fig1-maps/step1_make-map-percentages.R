...
data_2022only <- data[data$year ==2022, ]
Summarized_Table_AUD <- data_2022only %>%group_by(ADD_REGION) %>%
  summarize(N=n(), 
...
#to get %AUD:
datap_2022only <- datap[datap$year ==2022, ]
genpopN <- datap_2022only %>%group_by(ADD_REGION) %>%
  summarize(N=n())
Summarized_Table_AUD$N/genpopN$N